...
from nuclear.sublog import add_context, log

from trimmer.metadata import extract_artist_title


def tag_mp3(mp3_file: str, artist: str, title: str):
    with add_context('tagging mp3', artist=artist, title=title, mp3_file=mp3_file):
...
def read_mp3_artist_title(mp3_file: str) -> Tuple[str, str]:
    with add_context('extracting mp3 metadata', mp3_file=mp3_file):
...
def read_mp3_tags(mp3_file: str) -> Tuple[str, str]:
    with add_context('reading mp3 tags'):
        audiofile = eyed3.load(mp3_file)
...
def extract_filename_artist_title(mp3_file: str) -> Tuple[str, str]:
    with add_context('extracting metadata from filename'):
