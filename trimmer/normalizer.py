...
def normalize_song(mp3_file: str, no_trim: bool, no_fade: bool, no_normalize: bool,
    ...
    with add_context('normalizing mp3', mp3_file=mp3_file):
        ...
        if not no_normalize:
            if user_gain is not None:
                ...
                logger.info('normalizing volume level...', volume=f'{volume:.2f}dB', dBFS=f'{song.dBFS:.2f}dB')
                gain = -volume
            song = song.apply_gain(gain)
            logger.info('volume normalized', gain=f'{gain:.2f}dB')
        ...
        if not no_trim:
            ...
            logger.info('silence trimmed', trim_start=duration_to_human(start_trim), trim_end=duration_to_human(end_trim),
                     duration_before=duration_to_human(pre_duration), duration_after=duration_to_human(post_duration))

        if not no_fade:
            fade_in_duration = 100
            fade_out_duration = 1000
            logger.info('applying fade-in & fade-out...',
        ...
        duartion = len(song)
        logger.info('saving song...', mp3_file=mp3_file, duration=duration_to_human(duartion))
